-- ============================================================
-- 054_smart_multi_package_checkin.sql
-- Nâng cấp hàm checkin_pass_and_issue_ticket:
-- 1. Smart Selection: Ưu tiên gói đang dùng (IN), FIFO cho gói dự trữ (UNUSED)
-- 2. Gom nhóm đại diện theo ticket_type_id để giảm popup
-- 3. Trả thêm sport_type để frontend phân nhóm UI
-- ============================================================
...
DECLARE
    ...
    v_smart_tickets JSONB := '[]'::jsonb;
    v_smart_count INTEGER := 0;
    v_requires_gate_ticket BOOLEAN := true;
BEGIN
    ...
    IF p_selected_ticket_id IS NOT NULL THEN
        SELECT t.*, tt.category, tt.name AS type_name, tt.validity_days, tt.duration_months, tt.duration_unit, tt.lesson_class_type, COALESCE(tt.requires_gate_ticket, true) AS requires_gate_ticket, COALESCE(tt.sport_type, 'SWIMMING') AS sport_type
        INTO v_ticket
        ...
        v_requires_gate_ticket := v_ticket.requires_gate_ticket;
    ...
    -- =========================================================================
    -- PHẦN B: QUÉT TÌM CÁC GÓI (Smart Selection)
    -- =========================================================================
    ELSE
        -- B.1: Lấy TẤT CẢ gói khả dụng (giữ nguyên query cũ)
        SELECT jsonb_agg(
            jsonb_build_object(
                'id', t.id,
                ...
                'sport_type', COALESCE(tt.sport_type, 'SWIMMING')
        ...
        -- =====================================================================
        -- B.2: SMART SELECTION — Nhiều gói (v_available_count > 1)
        -- =====================================================================
        -- B.2: SMART SELECTION — Nhiều gói (v_available_count > 1)
        -- Thuật toán: Cho mỗi ticket_type_id, chọn 1 đại diện:
        --   - Ưu tiên gói ĐANG HOẠT ĐỘNG (is_active = true)
        --   - Nếu cùng loại đều UNUSED → lấy gói cũ nhất (FIFO theo sold_at)
        -- =====================================================================
        
        SELECT jsonb_agg(representative), count(*)
        INTO v_smart_tickets, v_smart_count
        FROM (
            SELECT DISTINCT ON ((elem->>'ticket_type_id')) elem AS representative
            FROM jsonb_array_elements(v_available_tickets) AS elem
            ORDER BY 
                (elem->>'ticket_type_id'),
                -- Ưu tiên gói đang hoạt động trước (is_active = true → 0, false → 1)
                (CASE WHEN (elem->>'is_active')::boolean THEN 0 ELSE 1 END) ASC,
                -- Trong cùng trạng thái, lấy gói cũ nhất
                (elem->>'sold_at') ASC
        ) sub;

        -- Nếu chỉ còn 1 đại diện → check-in luôn (không cần popup)
        IF v_smart_count = 1 THEN
            DECLARE
                v_auto_id UUID := (v_smart_tickets->0->>'id')::UUID;
            BEGIN
                RETURN public.checkin_pass_and_issue_ticket(p_pass_id, p_staff_id, p_confirm_new_package, v_auto_id);
            END;
        END IF;

        -- Nhiều đại diện (khác loại) → hiện popup phân nhóm
        RETURN jsonb_build_object(
            'success', false,
            'needs_package_selection', true,
            'message', 'Khách có nhiều loại gói/dịch vụ. Vui lòng hỏi khách muốn sử dụng dịch vụ nào hôm nay.',
            'available_packages', v_smart_tickets
        );
    ...
    -- =========================================================================
    -- TẠO VÉ LƯỢT QUA CỔNG (chỉ khi requires_gate_ticket = true)
    -- =========================================================================
    v_tickets_issued := '[]'::jsonb;

    IF v_requires_gate_ticket THEN
        -- Tạo vé lượt phụ 1
    ...
    -- =========================================================================
    -- GHI LOG CHECK-IN (wrapped in exception block to not fail main flow)
    -- =========================================================================
    BEGIN
        INSERT INTO public.checkin_logs (
            source_ticket_id, gate_ticket_id, customer_id, customer_name, card_code,
            ticket_type_name, ticket_category, sport_type, remaining_sessions,
            is_new_activation, requires_gate_ticket, checked_in_by
        ) VALUES (
            v_ticket.id,
            CASE WHEN v_requires_gate_ticket THEN v_new_ticket_id ELSE NULL END,
            v_ticket.customer_id,
            v_ticket.customer_name,
            COALESCE(v_ticket.card_code, ''),
            COALESCE(v_ticket.type_name, 'N/A'),
            COALESCE(v_ticket.category::TEXT, ''),
            COALESCE(v_ticket.sport_type, 'SWIMMING'),
            CASE WHEN v_ticket.remaining_sessions IS NOT NULL THEN v_ticket.remaining_sessions - 1 ELSE NULL END,
            v_is_new_package,
            v_requires_gate_ticket,
            p_staff_id
        );
    EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'checkin_logs INSERT failed: %', SQLERRM;
    END;
    ...
    RETURN jsonb_build_object(
        ...
        'requires_gate_ticket', v_requires_gate_ticket,
