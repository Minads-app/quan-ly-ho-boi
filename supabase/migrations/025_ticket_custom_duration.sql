-- Thêm cột để lưu thời hạn tùy chỉnh (VD: khách đăng ký lớp học bơi 1 kèm 1 có thời hạn 3 tháng)
-- Các vé trả trước / thẻ tháng / khóa học sẽ chưa có hiệu lực (valid_from = NULL, valid_until = NULL) khi mua, 
-- và chỉ bắt đầu tính thời hạn khi check-in lần đầu tiên.

ALTER TABLE public.tickets
ADD COLUMN IF NOT EXISTS custom_duration_months NUMERIC(4,1),
ADD COLUMN IF NOT EXISTS custom_validity_days INTEGER;
...
BEGIN
    ...
    ELSE
        ...
        -- TRƯỜNG HỢP 2: Có > 1 gói khả dụng -> YÊU CẦU LỄ TÂN CHỌN
