-- Migration 043: Fix checkin ticket type fallback logic
-- Resolves "null value in column ticket_type_id of relation tickets violates not-null constraint"
-- During check-in, the system tries to create a 'Vé Lượt Trả Trước' or daily ticket. If the exact name
-- is missing, it will now fallback to any active DAILY ticket type to avoid null constraint errors.
...
BEGIN
    ...
    ELSE
        ...
        FROM public.tickets t
        JOIN public.ticket_types tt ON t.ticket_type_id = tt.id
        WHERE (v_pass_uuid IS NOT NULL AND t.id = v_pass_uuid OR UPPER(t.card_code) = p_pass_id)
