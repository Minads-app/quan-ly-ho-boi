-- ============================================================
-- 048_fix_phantom_checkin_packages.sql
-- Fix: Đánh dấu sub-ticket check-in bằng source='CHECKIN'
-- để không hiển thị như gói riêng biệt trong trang Khách Hàng
-- ============================================================

-- 1. Mở rộng CHECK constraint: thêm 'CHECKIN'
...
ALTER TABLE public.tickets ADD CONSTRAINT tickets_source_check
    CHECK (source IN ('POS', 'IMPORT', 'CHECKIN'));

-- 2. Sửa dữ liệu cũ: đánh dấu các sub-ticket check-in đã tồn tại
-- Nhận diện: price_paid=0, total_sessions=1, remaining_sessions=1,
-- valid_from = valid_until, và ticket_type thuộc MULTI/MONTHLY/LESSON
UPDATE public.tickets t
SET source = 'CHECKIN'
FROM public.ticket_types tt
WHERE t.ticket_type_id = tt.id
  AND tt.category IN ('MULTI', 'MONTHLY', 'LESSON')
  AND t.price_paid = 0
  AND t.total_sessions = 1
  AND t.remaining_sessions IS NOT NULL
  AND t.valid_from IS NOT NULL
  AND t.valid_from = t.valid_until
  AND t.source = 'POS';

-- 3. Cập nhật hàm checkin_pass_and_issue_ticket: set source='CHECKIN'
...
BEGIN
        ...
        IF v_available_count = 0 THEN
            ...
            BEGIN
                ...
                ELSIF v_any_ticket.category = 'DAILY' THEN
                    RETURN jsonb_build_object('success', false, 'message', 'Đây là vé lượt, không phải thẻ tháng/nhiều buổi/học bơi. Vui lòng hướng dẫn khách mua vé nếu muốn vào cổng tiếp.');
                ELSE
    ...
    IF v_ticket.category = 'LESSON' THEN
        IF v_ticket.lesson_class_type = 'ONE_ON_TWO' THEN
             v_checkin_type_name := 'Học Bơi 1 Kèm 2 - ' || v_ticket.type_name;
        ELSIF v_ticket.lesson_class_type = 'GROUP' THEN
             v_checkin_type_name := 'Học Bơi Nhóm - ' || v_ticket.type_name;
        ELSE
             v_checkin_type_name := 'Học Bơi 1 Kèm 1 - ' || v_ticket.type_name;
        END IF;
    ELSIF v_ticket.category = 'MULTI' THEN
        v_checkin_type_name := 'Vé Nhiều Buổi - ' || v_ticket.type_name;
    ELSIF v_ticket.category = 'MONTHLY' THEN
        v_checkin_type_name := 'Vé Tháng - ' || v_ticket.type_name;
    ELSE
        v_checkin_type_name := v_ticket.type_name;
    END IF;

    -- Tạo vé lượt phụ 1, source='CHECKIN' để phân biệt với vé gốc
    INSERT INTO public.tickets (
        ticket_type_id, status, customer_name, customer_phone, valid_from, valid_until, sold_by, price_paid, remaining_sessions, total_sessions, customer_id, card_code, order_id, source
    ) VALUES (
        v_ticket.ticket_type_id, 'UNUSED'::public.ticket_status, v_ticket.customer_name, v_ticket.customer_phone, v_today, v_today, p_staff_id, 0, 1, 1, v_ticket.customer_id, v_ticket.card_code, v_ticket.order_id, 'CHECKIN'
    ...
    IF v_ticket.category = 'LESSON' AND v_ticket.lesson_class_type = 'ONE_ON_TWO' THEN
        INSERT INTO public.tickets (
            ...
            v_ticket.ticket_type_id, 'UNUSED'::public.ticket_status, COALESCE(v_ticket.customer_name_2, 'Học viên 2'), v_ticket.customer_phone, v_today, v_today, p_staff_id, 0, 1, 1, v_ticket.customer_id, v_ticket.card_code, v_ticket.order_id, 'CHECKIN'
