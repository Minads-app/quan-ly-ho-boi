-- ============================================================
-- 044_fix_qr_case_sensitivity.sql
-- Fix case sensitivity for QR scanning gate check-in
-- ============================================================
...
DECLARE
    ...
    v_close_time    TIME;
BEGIN
    -- Fix caps lock barcode scanning issues
    ...
    BEGIN
        ...
        WHERE t.id = v_ticket_id OR UPPER(t.card_code) = p_ticket_code
    ...
    EXCEPTION WHEN invalid_text_representation THEN
        ...
        FROM public.tickets t
        JOIN public.ticket_types tt ON t.ticket_type_id = tt.id
        WHERE UPPER(t.card_code) = p_ticket_code
        ORDER BY t.created_at DESC
        LIMIT 1
        FOR UPDATE;
    END;
