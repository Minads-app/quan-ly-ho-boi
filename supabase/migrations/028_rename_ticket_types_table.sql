-- Migration 028: Đổi tên ticket types trong bảng ticket_types
-- Chạy trên Supabase SQL Editor

-- Đổi tên vé check-in từ thẻ
UPDATE public.ticket_types SET name = 'Vé Lượt Trả Trước' WHERE name = 'Vé Lượt (Từ Thẻ)';

-- Đổi tên vé học bơi (tất cả các tên cũ có thể tồn tại)
UPDATE public.ticket_types SET name = 'Vé Học Bơi 1 Kèm 1' WHERE name IN ('VÉ HỌC BƠI', 'VÉ HỌC BƠI 1:1');
UPDATE public.ticket_types SET name = 'Vé Học Bơi 1 Kèm 2' WHERE name = 'VÉ HỌC BƠI 1 KÈM 2';
UPDATE public.ticket_types SET name = 'Vé Học Bơi Nhóm' WHERE name = 'VÉ HỌC BƠI NHÓM';

-- Cập nhật function để tham chiếu tên mới
...
BEGIN
    -- *** THAY ĐỔI: Dùng tên mới ***
    SELECT id INTO v_new_ticket_type_id FROM public.ticket_types WHERE name = 'Vé Lượt Trả Trước' LIMIT 1;
