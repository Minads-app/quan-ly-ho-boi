...
export default function InventoryPage() {
    ...
    const printInventorySlip = (mode: 'IMPORT' | 'EXPORT', items: SlipItem[], note: string) => {
        ...
        const htmlParts = [
            ...
            '<style>',
            '@page { size: ' + (isA5 ? 'A5' : '80mm auto') + '; margin: ' + (isA5 ? '10mm' : '0') + '; }',
            '@media screen { body { width: ' + (isA5 ? '100%' : '80mm') + '; max-width: ' + (isA5 ? '148mm' : '80mm') + '; aspect-ratio: ' + (isA5 ? 'auto' : '4/3') + '; margin: 0 auto; overflow: hidden; } }',
            '@media print { * { color: #000 !important; background: transparent !important; filter: grayscale(100%) !important; } }',
            '* { margin: 0; padding: 0; box-sizing: border-box; }',
            'body { font-family: "Times New Roman", Times, serif; width: 100%; max-width: ' + (isA5 ? '148mm' : '320px') + '; margin: 0 auto; padding: ' + (isA5 ? '20px' : '16px') + '; font-size: ' + (isA5 ? '14px' : '13px') + '; color: #000; background: #fff; line-height: 1.4; }',
            ...
            '.items-table { width: 100%; border-collapse: collapse; margin-top: 16px; margin-bottom: 20px; font-size: ' + (isA5 ? '14px' : '12px') + '; }',
            '.items-table th, .items-table td { border: 1px solid #000; padding: ' + (isA5 ? '8px' : '4px') + '; }',
            '.items-table th { font-weight: bold; background: #f0f0f0; }',
            '.footer { text-align: center; margin-top: 32px; font-size: ' + (isA5 ? '14px' : '12px') + '; font-style: italic; display: flex; justify-content: space-around; }',
            '.footer div { width: 45%; }',
            '</style>',
            '</head>',
            '<body>',
            ...
            '<div class="info-item"><span>Mã phiếu:</span> <strong>' + slipId + '</strong></div>',
            '<div class="info-item"><span>Thời gian:</span> <span>' + currentDate + '</span></div>',
            '<div class="info-item"><span>Người lập:</span> <span>' + (profile?.full_name || 'Admin') + '</span></div>',
            note ? '<div class="info-item" style="width: 100%;"><span>Ghi chú:</span> <span>' + note + '</span></div>' : '',
            '</div>',
            '<table class="items-table">',
            '<thead><tr><th style="width: 40px;">STT</th><th>Sản phẩm</th><th style="width: 50px;">ĐVT</th><th style="width: 60px;">Số lượng</th></tr></thead>',
            '<tbody>' + trs + '</tbody>',
            '</table>',
            '<div class="footer">',
            '<div><p>Người giao/nhận</p><br><br><br><p>(Ký, ghi rõ họ tên)</p></div>',
    ...
    const printAuditSlipWithData = async (auditId: string, note: string | null) => {
        ...
        const htmlParts = [
            ...
            '@media print { * { color: #000 !important; background: transparent !important; filter: grayscale(100%) !important; } }',
            '* { margin: 0; padding: 0; box-sizing: border-box; }',
            'body { font-family: "Times New Roman", Times, serif; width: 100%; max-width: 297mm; margin: 0 auto; padding: 20px; font-size: 15px; color: #000; background: #fff; line-height: 1.4; }',
            '.text-center { text-align: center; }',
            '.mb-2 { margin-bottom: 8px; }',
            '.mb-4 { margin-bottom: 24px; }',
            'h1 { font-size: 26px; text-transform: uppercase; margin-bottom: 8px; }',
            'h2 { font-size: 22px; text-transform: uppercase; margin-top: 16px; margin-bottom: 20px; text-align: center; border-bottom: 2px solid #000; padding-bottom: 8px; display: inline-block; }',
            '.header-container { text-align: center; margin-bottom: 24px; }',
            '.info-container { display: flex; flex-wrap: wrap; margin-bottom: 16px; justify-content: space-between; gap: 10px; }',
            '.info-item { width: 48%; margin-bottom: 6px; }',
            '.items-table { width: 100%; border-collapse: collapse; margin-top: 20px; margin-bottom: 24px; font-size: 15px; }',
            '.items-table th, .items-table td { border: 1px solid #000; padding: 10px 8px; }',
            '.items-table th { font-weight: bold; background: #f0f0f0; text-align: center; font-size: 14px; }',
            '.footer { text-align: center; margin-top: 40px; font-size: 15px; font-style: italic; display: flex; justify-content: space-around; }',
            '.footer div { width: 45%; }',
            '</style>',
            '</head>',
            '<body>',
            '<div class="header-container">',
            bizInfo.business_logo ? '<img src="' + bizInfo.business_logo + '" style="max-height: 60px; margin-bottom: 12px;" />' : '',
            '<h1>' + (bizInfo.business_name || 'Hệ Thống Vé Bơi') + '</h1>',
            bizInfo.business_address ? '<div style="font-size: 14px;">' + bizInfo.business_address + '</div>' : '',
            '<h2 class="text-center">' + slipTitle + '</h2>',
            '</div>',
            '<div class="info-container">',
            '<div class="info-item"><span>Mã phiếu:</span> <strong>' + auditId.substring(0, 8).toUpperCase() + '</strong></div>',
            '<div class="info-item"><span>Thời gian:</span> <span>' + currentDate + '</span></div>',
            '<div class="info-item"><span>Người lập:</span> <span>' + (profile?.full_name || 'Admin') + '</span></div>',
            note ? '<div class="info-item" style="width: 100%;"><span>Ghi chú:</span> <span>' + note + '</span></div>' : '',
            '</div>',
            '<table class="items-table">',
            '<thead><tr><th style="width: 50px;">STT</th><th>Sản phẩm / Diễn giải</th><th style="width: 80px;">ĐVT</th><th style="width: 120px;">Tồn hệ thống</th><th style="width: 120px;">Tồn thực tế</th><th style="width: 120px;">Chênh lệch</th></tr></thead>',
            '<tbody>' + trs + '</tbody>',
            '</table>',
            '<div class="footer">',
            '<div><p>Quản lý</p><br><br><br><br><p>(Ký, ghi rõ họ tên)</p></div>',
